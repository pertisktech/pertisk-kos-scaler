...
impl Default for CooldownConfig {
    fn default() -> Self {
        Self { scale_up: default_up_cooldown(), scale_down: default_down_cooldown() }
    }
}
...
impl Config {
    pub fn load(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let mut config: Self = serde_yaml::from_str(&contents).context("parsing configuration YAML")?;
...
fn default_sync_interval() -> Duration { Duration::from_secs(30) }
fn default_true() -> bool { true }
fn default_min_size() -> i32 { 1 }
fn default_max_size() -> i32 { 10 }
fn default_pending_pods() -> i32 { 1 }
fn default_target_utilization() -> f64 { 0.5 }
fn default_safe_buffer() -> f64 { 0.1 }
fn default_up_cooldown() -> Duration { Duration::from_secs(120) }
fn default_down_cooldown() -> Duration { Duration::from_secs(600) }
